...
def stream_keywords(keywords, rt, _out_file)
...
opts.each do |opt, arg|
  ...
  when '--help'
    puts <<-EOF
      ...
      -s, --stream
        real-time download of tweets using the streaming API
      -d, --download
        fetch the latest tweets
      -r, --retweets
        if passed, retweets will also be fetched
      -o, --out
        file to save to
        will attempt to save to db if not passed
    EOF
    exit 0
  end
end
